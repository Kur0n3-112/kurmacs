;; Filter by - to see only emacs config related things, and filter by > to see packages related things.

;;;- CONFIG
(add-to-list 'load-path "~/.config/emacs/emacs.local/") ;; Loads other files for using in config
...
;; To full gui mode
;;(scroll-bar-mode 1) ;; Enable visible scroolbar
;;(tool-bar-mode 1)   ;; Enable the toolbar
...
(defun kur/copy-current-directory-for-cd ()
  "Copy the current directory to Wayland clipboard in a format ready for terminal cd command."
  (interactive)
  (let* ((current-dir (if (buffer-file-name)
                         (file-name-directory (buffer-file-name))
                       default-directory))
         (cd-command (format "cd %s" (shell-quote-argument current-dir))))
    (with-temp-buffer
      (insert cd-command)
      (call-process-region (point-min) (point-max) "wl-copy"))
    (message "Copied to clipboard: %s" cd-command)))

(global-set-key (kbd "C-c d") 'kur/copy-current-directory-for-cd) ;; Bind to C-c d

;;- Creation of binding to use compile and recompile faster
(define-prefix-command 'kur/custom-prefix-compile)
(global-set-key (kbd "C-x c") 'kur/custom-prefix-compile)

(define-key kur/custom-prefix-compile (kbd "c") 'compile)
(define-key kur/custom-prefix-compile (kbd "r") 'recompile)

;;- simpc-mode
;; Adding `/path/to/simpc` to load-path so `require` can find it
(add-to-list 'load-path "~/.config/emacs/emacs.local/simpc-mode.el")
...
;; Alias simpc-mode to c-mode to make lsp pick up simpc-mode as the default c-mode
(with-eval-after-load 'lsp-mode
  (add-to-list 'lsp-language-id-configuration '(simpc-mode . "c")))

(add-hook 'simpc-mode-hook #'lsp)

;;- Emacs backup files management
...
;;> ivy: better M-x and things like that completions
(use-package ivy
  :diminish
  :bind (("C-s" . swiper)
...
;;> which-key: Enable the lookup from all the keybinds on emacs, for example when using M-x and holding
...
;; To use the find-file without counsel-mode and ivy-mode
(defun kur/find-file()
  (interactive)
  (ivy-mode -1)
  (counsel-mode -1)
...
(global-set-key (kbd "C-x C-f") 'kur/find-file)

(defun kur/switch-to-buffer()
  (interactive)
  (ivy-mode -1)
  (counsel-mode -1)
...
(global-set-key (kbd "C-x b") 'kur/switch-to-buffer)
...
;;> doom-themes: Theming with doom-themes
...
;;> Evil-mode: vim keybinds on emacs
(use-package evil
  :init
...
;;> evil-collection: Enhance evil mode with more human keybinds
...
;; Org-roam config (for taking notes)
...
(use-package lsp-mode
  ...
  :custom
    ...
    (lsp-rust-analyzer-display-lifetime-elision-hints-enable "skip_trivial") ;; Whether to show elided lifetime inlay hints.
    (lsp-rust-analyzer-display-chaining-hints t)
    (lsp-rust-analyzer-display-lifetime-elision-hints-use-parameter-names nil) ;; When showing elided lifetime inlay hints, whether to use parameter names or numeric placeholder names for the lifetimes.
    (lsp-rust-analyzer-display-closure-return-type-hints t)
    (lsp-rust-analyzer-display-parameter-hints nil)
    (lsp-rust-analyzer-display-reborrow-hints nil)
    (lsp-headerline-breadcrumb-icons-enable nil) ;; Disable the icons
    (lsp-modeline-code-action-icons-enable nil) ;; Disable the icons
  :commands lsp)
...
;;  (require 'dap-node) ;; Set up DAP-mode for Rust
;;  (dap-node-setup))
...
;;> Projectile for more intelingence to use projects
...
(setq hunspell-default-dict "es_ES")
...
;;> DAP-mode (Debbuger Adaptor Protocol Mode)
;; (use-package dap-mode)
...
;;                                        :environment []
;;                                        :program "${workspaceFolder}/target/debug/ replace with binary "
;;                                        :cwd "${workspaceFolder}"
;;                                        :console "external"
;;                                        :dap-compilation "cargo build"
;;                                        :dap-compilation-dir "${workspaceFolder}"))

;; ;; DAP-mode Dependencies and Modules activation:
;; (require 'dap-mode)
;; (require 'dap-gdb)
;; (require 'dap-lldb)
